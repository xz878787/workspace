...
function App() {
  ...
  useEffect(() => {
    ...
    const onMessageReceived = (e) => {
      const data = e.data;
      // #region debug-point main-thread-receive
      console.log("[DEBUG] main thread received:", data.status, data.file || "", data.progress !== undefined ? `progress=${data.progress}` : "");
      // #endregion
      switch (data.status) {
        case "checked":
          break;
        ...
        case "initiate":
          // Filter out the aggregate "total" item, only show per-file progress
          if (data.file !== "total") {
        ...
        case "update":
          {
            const { output, tps: newTps, numTokens: newNumTokens, state } = data;
            setTps(newTps);
            ...
            setMessages((prev) => {
              ...
              if (msgData.answerIndex === undefined && state === "answering") {
                msgData.answerIndex = last.content.length;
              }
              cloned[cloned.length - 1] = msgData;
              return cloned;
            });
          }
          break;
